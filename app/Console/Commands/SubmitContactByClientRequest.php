...
class SubmitContactByClientRequest extends Command
{
     ...
     * @var string
     */
    protected $signature = 'submit:contact-client';
    ...
    public function handle()
    {
        ...
        foreach ($companyContacts as $companyContact) {
            ...
            if (!$crawler) {
                $this->updateCompanyContact($companyContact, self::STATUS_RETRY, 'Cannot get content from url');
                continue;
            }
            ...
            if (!$hasContactForm) {
                $this->updateCompanyContact($companyContact, self::STATUS_RETRY, 'Contact form not found');
                continue;
            }
            ...
            if ($isFooterInvalid) {
                $this->updateCompanyContact($companyContact, self::STATUS_RETRY, 'Content is unacceptable');
                continue;
            }

            if (!$this->form->getValues()) {
                $this->updateCompanyContact($companyContact, self::STATUS_RETRY, 'Form does not have values');
                continue;
            }
